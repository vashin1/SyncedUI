	...
	if sync == "BossEngaged" and rest == "The Bug Family" then
		if not started then
			if self.db.profile.panic then
				self:TriggerEvent("BigWigs_StartBar", self, L["panic_bar"], 20.4, "Interface\\Icons\\Spell_Shadow_DeathScream", true, "white")
				self:ScheduleEvent("PanicAnnounce", "BigWigs_Message", 17.4, L["panic_message"], "Urgent", true, "Alarm")
			end
